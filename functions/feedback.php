...
function brainworks_comment($comment, $args, $depth) {
  ...
  <?php endif; ?>
    <div class="comment-author vcard">
      ...
      <div>
        ...
        <p class="comment-meta commentmetadata"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>"><?php printf('%1$s ' . __('at', 'brainworks') . ' %2$s', get_comment_date(), get_comment_time()) ?></a></p>
        <?php if ($comment->comment_approved == '0') : ?>
          <p><em class="comment-awaiting-moderation"><?php _e('Your comment is awaiting moderation.', 'brainworks') ?></em></p>
        <?php endif; ?>
      </div>
    </div>
    <div class="row">
